...
```
int x = 10;const int a = x;     // ✅ 合法！运行期把 x 的值（10）存入只读变量 a // constexpr int b = x; // ❌ 编译直接暴毙！因为 x 是运行期变量，constexpr 在编译期根本拿不到它的值！
```
------------------------------
...
```
#include <iostream>
// 🔑 工业范式：声明为 constexpr 函数（C++14 起内部允许写循环和 if 分支）constexpr unsigned long long compile_time_factorial(int n) {
...
// 编译期在内存中直接固化出一个三维点，运行期零开销，天然支持线程安全constexpr Vec3 c_origin(0.0, 0.0, 0.0); 
```
------------------------------
